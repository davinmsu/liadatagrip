...
--- Валера - без джойна v1
SELECT
    month,
    user_id,
    groupArray(a1) as a1_arr,
    groupArray(a2) as a2_arr
FROM (    
    SELECT
        dateTrunc('month', timestamp) as month,
        user_id,
        if(reaction = '4bb23d6c-c1d0-4ed5-86d4-02a95bf453dd', ntext, NULL) as a1,
        if(reaction = '14e512ac-377d-4192-9b76-cb9815442bbc', ntext, NULL) as a2
    FROM (
        SELECT *
        FROM (
            SELECT
    ...
    WHERE reaction in (
        ...
        AND (isNotNull(a1) OR isNotNull(a2))
)
